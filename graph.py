...
class CovidGraph:
    ...
    def add_news(self, country_code, title, date, url_source, keywords=None):
        news_uri = URIRef(f"{PATH}/news/{country_code}/{self.news_id}")
        ...
        self.graph.add((news_uri, URIRef(f"{PATH}/properties/PublishedIn"), Literal(country_code)))

        self.news_id += 1

    def add_article(self, title, authors, abstract, date, url_source, citation="", art_type="", keywords=None):
        ...
        if keywords:
            for k in keywords:
                self.graph.add((article_uri, SDO.keywords, Literal(k)))
        else:
            self.graph.add((article_uri, SDO.keywords, Literal("")))
